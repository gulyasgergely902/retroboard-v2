<template>
  <div
    class="inline-flex items-center mr-2 ml-1 w-3/4"
    id="innerDiv"
  >
    ...
    <div class="overflow-x-auto overflow-y-hidden whitespace-nowrap h-12 fade-right no-scrollbar">
      <button
        ...
        :class="[categoryToHighlight === category.id ? 'bg-sky-500 dark:bg-slate-600' : 'bg-sky-100 dark:bg-slate-800']"
        class="text-sky-950 hover:bg-sky-300 dark:text-sky-50 dark:hover:bg-slate-500 hover:text-slate-950 dark:hover:text-sky-50 px-4 py-2 text-sm font-medium rounded-xl focus:z-10 focus:ring-2 focus:ring-sky-700 focus:text-sky-700 dark:focus:ring-sky-500 dark:focus:text-white m-1 transition-colors cursor-pointer"
      >
        {{ category.name }}
      </button>
    </div>
